...
angular.module('ActiveResource').provider('ARGET', function () {
    this.$get = [
        ...
        function ($http, deferred, Associations, Helpers, URLify, $q) {
            function resolveSingleGET(data, terms, options, count) {
            ...
            ;
            function resolveMultiGET(data, terms, options, count) {
                if(count)
                {
                    return {
                        count: count,
            ...
            return function generateGET(instance, url, terms, options) {
                ...
                return $http.get(url, config).then(function (response) {
                    ...
                    //CUSTOM MODIFIED -- PAGINATION
                    var originalData = data, count = null;
                    if(data.hasOwnProperty('rows') && data.hasOwnProperty('count'))
                    {
                        count = data.count;
                        data = data.rows;
                    }
                    ...
                    if (options.multi)
                        return resolveMultiGET(data, terms, options, count);
                    else
                        return resolveSingleGET(data, terms, options, count);
                });
            };
        }
    ];
            ...
            function Base() {
                ...
                _this.where = function (terms, options) {
                    ...
                    return GET(_this, url, terms, options).then(function (json) {
                        //CUSTOM MODIFIED: support pagination
                        var originalData = json, count = null;

                        if(json.hasOwnProperty('count') && json.hasOwnProperty('rows')){
                            count = json.count;
                            json = json.rows;
                        }
