...
(function(module) {
    ...
    module.init = function(beaver) {
        ...
        redisClient = redis.createClient({
            ...
            database: beaver.config.adapters.Redis.db,
            password: beaver.config.adapters.Redis.pass
        });

        redisClient.on("error", function (error) {
            var errorMsg = 'Failed to connect to Redis: ' + error.stack;
            beaver.winston.error(errorMsg);
            return deferred.reject(errorMsg);
        });

        redisClient.on("connect", function () {
            redisClient.select(beaver.config.adapters.Redis.db, function(err, data){
                if(err) return deferred.reject(err.stack || err);
                beaver.winston.info('Connect to Redis successfully', beaver.config.adapters.Redis.db);
